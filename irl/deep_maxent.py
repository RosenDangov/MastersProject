...
def irl(structure, feature_matrix, n_actions, discount, transition_probability,
    ...
    # Initialise alphaWeight weight, betaBias bias.
    if initialisation == "normal":
        alphaWeight = th.shared(rn.normal(size=(1, structure[-1])), name="alpha",
                      borrow=True)
    else:
        alphaWeight = th.shared(rn.uniform(size=(1, structure[-1])), name="alpha",
                      borrow=True)
    hist_alphaWeight_grad = T.zeros(alphaWeight.shape)  # For AdaGrad.
    ...
    for W, b in zip(weights, biases):
        ...
        # phis[1] = phi1 etc.
    # Reward.
    r = alphaWeight.dot(phis[-1]).reshape((n_states,))
    ...
    updates = []
    alphaWeight_grad = phis[-1].dot(svf - expected_svf).T
    hist_alphaWeight_grad += alphaWeight_grad**2
    adj_alphaWeight_grad = alphaWeight_grad/(adagrad_epsilon + T.sqrt(hist_alphaWeight_grad))
    updates.append((alphaWeight, alphaWeight + adj_alphaWeight_grad*learning_rate))
